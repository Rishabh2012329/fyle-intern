    ...
    return(state.loading?
        ...
        <div>
            <div className='w-full grid grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-3'>
                {state[page]?.map((obj)=><Card name={obj.name}/>)}
            </div>
            <div className='w-max m-auto mt-8'>
                <button className={`${page==1&&"text-gray-300"} mr-2`} disabled={pages==1} onClick={() => older(page-1)}>
                    prev
                </button>
                {pages.map((num)=><button onClick={() => onPageChange(num)} className={`w-8 h-8 ${page==num?"bg-blue-400 text-white":"border border-gray-300"} rounded-sm`}>{num}</button>)}
